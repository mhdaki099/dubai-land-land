...
def setup_openai_api():
    ...
    if not api_key:
        ...
        st.info("""
        **To use this application, you need to set up your OpenAI API key:**
        
        **For Streamlit Cloud:**
        1. Go to your app settings
        2. Add a secret: `OPENAI_API_KEY = "your-api-key-here"`
...
# Only proceed with the app if API key is available
if not api_key:
...
def load_faq_data():
    ...
    if not os.path.exists("processed_data"):
        ...
        st.info("This app requires FAQ data to be processed first. Please contact the administrator to set up the data files.")
        return pd.DataFrame(), ["All Topics"]
    ...
    if os.path.exists(csv_path):
        try:
            ...
            st.error(f"❌ Error loading CSV data: {str(e)}")
            st.info("Please check if the CSV file is properly formatted.")
        ...
        try:
            ...
            st.error(f"❌ Error loading JSON data: {str(e)}")
            st.info("Please check if the JSON file is properly formatted.")
    
    # If no data files found, show helpful message
    st.error("📄 No FAQ data files found.")
    st.info("""
    **To use this application, you need to:**
    1. Process your Excel FAQ files first
    2. Upload the processed data files to the cloud deployment
    3. Or contact the administrator to set up the data
    
    **Expected files:**
    - `processed_data/dld_faq_data.csv` OR
