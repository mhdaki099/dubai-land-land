...
def generate_response(query, relevant_faqs, language):
    ...
    try:
        ...
        response = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": f"User query: {query}\n\nRelevant FAQ information:{faq_context}"}])        
...
def main():
    ...
    with st.sidebar:
        # Logo and branding
        st.image("https://www.dubailand.gov.ae/Content/images/logo.png", width=200)
        st.markdown("---")
        ...
        selected_topic = st.selectbox(
            ...
            help="Choose a specific topic to get more accurate answers"
        )
        ...
        # FAQ Data Info
        st.markdown('<div class="sidebar-header">ℹ️ FAQ Data</div>', unsafe_allow_html=True)
    ...
    <div style="margin-bottom: 0.5rem; color: #666; font-size: 0.9rem;">
        💡 You can ask questions in English or Arabic. For best results, be specific and clear in your question.
    </div>
    """, unsafe_allow_html=True)
        ...
        with st.spinner("Finding the answer..."):
            ...
            if st.session_state.debug_mode:
                ...
                <div style="margin-top: 1rem;">
                    ...
                    • Total FAQs found: {len(relevant_faqs)}<br><br>
                    <strong>Matching Details:</strong><br>
                """
                
                for i, faq in enumerate(relevant_faqs):
                    match_type = faq['debug_info']['match_type']
                    score = faq['debug_info']['text_score']
                    rank = faq['debug_info']['rank']
                    debug_info += f"""
